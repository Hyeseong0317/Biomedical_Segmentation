...
#### GAN은 큰 이미지를 생성하는데 한계점이 존재한다. GAN을 통해 특별한 생성 tasks인 이미지 디블러링, super-resolution, 클래스 시각화를 수행할 수 있다.
